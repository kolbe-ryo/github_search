...
class IsarQueryHistoryRepository implements QueryHistoryRepository {
  ...
  Future<void> delete(QueryHistory queryHistory) async {
    ...
    return isar.writeTxn(
      () async {
        final count = await isar.queryHistoryCollections.filter().queryStringEqualTo(queryString).deleteAll();
        logger.v(
  ...
  Stream<List<QueryHistory>> changes({required String queryString}) {
    ...
    return _queryBuilder(queryString: queryString).watch().map(
          (collections) => collections.map((collection) => QueryHistory.create(collection.queryString)).toList(),
        );
  }

  /// 検索結果のクエリービルダーを帰す
  QueryBuilder<QueryHistoryCollection, QueryHistoryCollection, QAfterLimit> _queryBuilder({
